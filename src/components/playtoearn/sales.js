...
import useTopSearchChart from "src/hooks/playtoearn/useTopSearchChart";
import useMostVisitedChart from "src/hooks/playtoearn/useMostVisitedChart";
...
const Sales = (props) => {
  ...
  const [chart, setChart] = useState("Most Searched Games");

  const { data: topSearchData } = useTopSearchChart(chart === "Most Searched Games" ? name : "");
  const { data: mostVisitedData } = useMostVisitedChart(chart === "Most Visited Games" ? name : "");

  const chartData = chart === "Most Searched Games" ? topSearchData : mostVisitedData;
        ...
        title={
          <FormControl sx={{ m: 1, minWidth: 80 }}>
            ...
            <Select value={chart} label="Chart" onChange={handleChangeChart}>
              <MenuItem value="Most Searched Games">Most Searched Games</MenuItem>
              <MenuItem value="Most Visited Games">Most Visited Games</MenuItem>
            </Select>
          </FormControl>
        }
      />
